...
/**
 * Example of using SERIAL_AUXONE to write "Hello World" using the IO abstraction.
 *
 * Since the driver is asynchronous we need to use statically allocated memory for string
 * because driver initiates transfer and then returns before the transmission is completed.
 *
...
static uint8_t example_SERIAL_AUXONE[12] = "Hello World!";

static void tx_cb_SERIAL_AUXONE(const struct usart_async_descriptor *const io_descr)
{
	/* Transfer completed */
}
...
void SERIAL_AUXONE_example(void)
{
	...
	usart_async_register_callback(&SERIAL_AUXONE, USART_ASYNC_TXC_CB, tx_cb_SERIAL_AUXONE);
	/*usart_async_register_callback(&SERIAL_AUXONE, USART_ASYNC_RXC_CB, rx_cb);
	usart_async_register_callback(&SERIAL_AUXONE, USART_ASYNC_ERROR_CB, err_cb);*/
	usart_async_get_io_descriptor(&SERIAL_AUXONE, &io);
	usart_async_enable(&SERIAL_AUXONE);

	io_write(io, example_SERIAL_AUXONE, 12);
}
